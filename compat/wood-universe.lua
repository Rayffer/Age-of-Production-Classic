if mods["early-agriculture"] then
  table.insert(data.raw["assembling-machine"]["assembling-machine-1"].crafting_categories, "woodworking", "organic")
end

if mods["wood-industry"] then
  if data.raw.recipe["heavy-oil-adsorption"] then
    data.raw.recipe["heavy-oil-adsorption"].categories = { "petrochemistry", "organic", "chemistry" }
  end
end
...
if mods["wood-logistics"] then
  if not mods["lignumis"] and settings.startup["wood-logistics-lumber-mill"] and settings.startup["wood-logistics-lumber-mill"].value then
    table.insert(data.raw["assembling-machine"]["aop-lumber-mill"].crafting_categories, "crafting", "carpentry")
